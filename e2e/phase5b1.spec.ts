...
test('Phase 5B-1 inventory and atlas show artwork only where supplied', async ({
  ...
  await expect(atlas.locator('[data-product-artwork]')).toHaveCount(27);
  await expect(atlas.locator('.atlas-artwork-wrap')).toHaveCount(27);
  await expect(atlas.locator('.atlas-product-card.no-artwork')).toHaveCount(21);
