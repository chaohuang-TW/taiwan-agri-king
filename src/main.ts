...
app.innerHTML = `
  ...
  <dialog id="atlas-dialog" aria-labelledby="atlas-title"><div class="dialog-head"><h2 id="atlas-title">農產圖鑑</h2><button type="button" data-close aria-label="關閉圖鑑">關閉</button></div><div class="atlas-grid">${PRODUCTS.map((product) => `<article class="atlas-product-card"><div class="atlas-artwork-wrap">${renderProductArtwork(product, 'atlas-artwork')}</div><div><span>${PRODUCT_CATEGORY_NAMES[product.category]}</span><h3>${product.name}</h3><p>${countyName(product.countyId)}</p><small>旺季：${product.peakSeasons.join('、')}</small></div></article>`).join('')}</div></dialog>
`;
